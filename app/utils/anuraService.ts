...
export const generateRoast = async (
  ...
  try {
    ...
    const walletValueFormatted = parseFloat(transactionSummary.totalValue) > 10 
    ...
    TOKENS TRADED: ${transactionSummary.topTokensTraded.length > 0 ? transactionSummary.topTokensTraded.map(t => t.symbol).join(', ') : 'None'}
    ...
    SPECIAL NOTES:
    ${transactionSummary.topTokensTraded.some(t => ['PEPE', 'SHIB', 'DOGE', 'FLOKI', 'BONK'].includes(t.symbol)) ? '- Trades meme coins' : ''}
    ${transactionSummary.topTokensTraded.some(t => ['UNI', 'SUSHI', 'CAKE', 'CRV'].includes(t.symbol)) ? '- Uses DeFi exchanges' : ''}
    ${transactionSummary.totalValue < 0.01 ? '- Almost empty wallet' : ''}
    ${transactionSummary.daysInactive > 30 ? '- Abandoned wallet' : ''}
    ${transactionSummary.failedTransactions > 3 ? '- Lots of failed transactions' : ''}
    ...
    if (responseText.includes('event:') || responseText.includes('data:')) {
      ...
      const completionMatch = responseText.match(/data:\s*\{"model":.*?"message".*?\}/s);
      ...
      const errorMatch = responseText.match(/error response from server: (.*?)($|\n)/s);
...
export const getRandomFallbackRoast = (
  ...
  const roast = set[uniqueParams.styleIndex % set.length || Math.floor(Math.random() * set.length)];
