  ...
  try {
    ...
    for (let i = 0; i < numberOfRoasts; i++) {
      ...
      const roastPromise = new Promise<RoastResult>(async (resolve) => {
        ...
        if (i > 0) {
          console.log(
            `Waiting ${delayMs}ms before starting request #${i + 1}...`
          )
        ...
        try {
          ...
          console.log(
            `Generating roast #${i + 1} with uniqueParams:`,
            uniqueParams
          )
          ...
          console.log(
            `Got successful response for roast #${i + 1} in ${
          ...
          const uniqueSeed =
            Date.now() + Math.floor(Math.random() * 1000000) + i * 31337
    ...
    console.log(
      `Waiting for ${roastPromises.length} roast promises to complete...`
    )
    ...
    console.log(
      `Got ${roastPromises.length} results with ${fallbackCount} fallbacks`
    )
    ...
    results.forEach((result, index) => {
      console.log(
        `Roast #${index + 1}: ${
          result.isFallback ? 'FALLBACK' : 'AI-GENERATED'
    ...
    for (let i = 0; i < 3; i++) {
      ...
      let fallbackRoast: string
      do {
        fallbackRoast = getRandomFallbackRoast({
          seed: uniqueSeed + usedIndices.size,
        })
      } while (fallbackRoasts.includes(fallbackRoast))
